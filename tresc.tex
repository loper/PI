...
Dla wysyłania:\\\fbox{rsync -\%parametry\% \%zrodlo\% [LOGIN@]HOST::MODUL} \\lub\\ \fbox{rsync -\%parametry\% \%zrodlo\% rsync://[LOGIN@]HOST[:PORT]/\%cel\%} podając oczywiście login, host i modul lub port i cel, do którego się odwołujemy.\\
...
\end{verbatim}
Mamy wtedy: \fbox{rsync -\%parametry\% \%zrodlo\% \%login\%@\%host\%::\%modul\%}, dzięki czemu wygląda to przejrzyściej, a my sami ustawiamy jedynie zmienne, nie martwiąc się o składnę polecenia.
